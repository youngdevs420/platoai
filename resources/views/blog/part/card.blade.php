<article class="shadow-[0_2px_4px_rgba(149,146,157,0.15)] rounded-2xl w-full flex flex-col">
    <figure>
        <a href="{{ url('/blog', $post->slug) }}">
            <img class="w-full h-40 object-cover rounded-t-2xl" src="/{{$post->feature_image}}" alt="{{$post->title}}">
        </a>
    </figure>
    <div class="p-5 min-h-[180px] flex flex-col font-medium">
        ...
        <h2 class="text-[21px] mb-4 tracking-tight leading-[26px]"><a href="{{ url('/blog', $post->slug) }}">{{$post->title}}</a></h2>
