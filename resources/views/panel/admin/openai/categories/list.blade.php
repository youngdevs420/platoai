...
@section('content')
    ...
    <div class="pt-6 page-body">
        <div class="container-xl">
            <div class="mb-2">
                <a @if(env('APP_STATUS') == 'Demo') onclick="return toastr.info('This feature is disabled in Demo version.')" @else href="{{ LaravelLocalization::localizeUrl( route('dashboard.admin.openai.categories.addOrUpdate') ) }}" @endif class="btn btn-success">{{__('Add Category')}}</a>
            </div>
            <div class="card">
				<div id="table-default" class="card-table table-responsive">
					<table class="table">
						...
						<tr>
							<th>{{__('Name')}}</th>
							<th class="text-right !pr-14">{{__('Action')}}</th>
						</tr>
						</thead>
